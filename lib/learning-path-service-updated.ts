 ...
 */
export async function fetchLearningPath(topic: string): Promise<LearningPathResponse | { isImage: true; imageUrl: string; topic: string }> {
  try {
    // The public URI of the AI bot
    const apiUrl = `https://7100-34-125-133-151.ngrok-free.app/learning-path`;
    
    // Since the API seems to be returning HTML or an image directly,
    // we'll use the URL directly as the image source with a query parameter
    console.log(`Using direct image URL with topic: ${topic}`);
    const imageUrl = `${apiUrl}?topic=${encodeURIComponent(topic)}`;
    
    // Return a special response indicating we're using the direct URL as an image source
    return {
      isImage: true,
      imageUrl,
      topic
        ...
        // Process the response data - handle different formats
        if (data && data.path && Array.isArray(data.path)) {
          // Format 1: Response with 'path' property containing an array of step titles
          return {
            success: true,
            topic,
            steps: data.path.map((stepTitle: string, index: number) => ({
              id: index + 1,
              title: stepTitle,
              description: `Step ${index + 1} in your journey to master ${topic}.`,
              resources: [
                `https://www.google.com/search?q=${encodeURIComponent(stepTitle)}`,
                `https://www.youtube.com/results?search_query=${encodeURIComponent(stepTitle)}`
              ],
              estimatedTimeMinutes: 60 + (index * 30) // Increasing time for later steps
            }))
          };
        } else if (data && Array.isArray(data)) {
          // Format 2: Direct array of steps
          return {
            success: true,
            topic,
    ...
    try {
      const getUrl = `${apiUrl}?topic=${encodeURIComponent(topic)}`;
